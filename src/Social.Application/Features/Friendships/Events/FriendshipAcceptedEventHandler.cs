    ...
    : IDomainEventHandler<FriendshipAcceptedEvent>
{
    public async Task Handle(FriendshipAcceptedEvent notification, CancellationToken cancellationToken)
    {
        ...
        var acceptor = await userProfileRepository.GetSingleAsync(new UserProfileWithAvatarSpec(notification.UserProfileId), cancellationToken);
        var instigator = await userProfileRepository.GetSingleAsync(new UserProfileByTagWithAvatarSpec(notification.FriendTag), cancellationToken);
